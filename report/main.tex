...
Autonomous navigation is a cornerstone of modern robotics, especially in environments where external positioning systems like GPS are unavailable. This challenge is exemplified by the NASA Lunabotics competition, in which Boise State University's Bender Robotics Team is competing with their fully autonomous lunar excavation robot, \textit{Bender}. The robot is tasked with navigating rough, dusty terrain, collecting regolith, and returning it to a specified location—entirely without human intervention.
...
In GPS-denied environments like the lunar surface, correcting this drift without an external reference requires accurate scan matching between consecutive LiDAR frames. Iterative Closest Point (ICP) algorithms offer a compelling solution, using geometric relationships between overlapping point clouds to estimate the robot's motion and update the map. However, ICP comes with trade-offs between computational cost and alignment accuracy, which are especially relevant for embedded platforms like the ESP32, Teensy 4.1, and Raspberry Pi 5, where real-time constraints demand sub-10ms compute cycles.

The objective of this work is to analyze and compare three major variants of the ICP algorithm—Point-to-Point, Point-to-Plane, and Generalized ICP—focusing on their mathematical formulation, numerical stability, and suitability for real-time implementation on constrained hardware. The aim is to determine which variant provides the best balance of performance and computational cost for potential deployment on the Bender robot during the final testing phase.
...
\]
This is solved using Singular Value Decomposition (SVD) on the cross-covariance matrix $H = \sum_i \mathbf{p}_i \mathbf{q}_i^\top$. Let $H = U\Sigma V^\top$, then $R = VU^\top$ ensures the closest orthogonal matrix in the least squares sense.
...
\]
The overdetermined system is efficiently solved with QR decomposition or normal equations. Your implementation supports both QR and SVD-based updates depending on the ICP variant.
...
\begin{itemize}
  ...
  \item Visualization: Matplotlib for Python, custom C++ visualization using OpenGL
\end{itemize}
